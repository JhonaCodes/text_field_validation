...
void main() {
  group("CRYPTO_ADDRESS_VALIDATION_TEST", () {
    ...
    test("bitcoin_address_empty_validation_test", () {
      expect(TextFieldValidation.bitcoinAddress(""),
          'Bitcoin address is required');
    });

    test("bitcoin_address_invalid_test", () {
      expect(TextFieldValidation.bitcoinAddress("invalidBitcoinAddress"),
          'Please enter a valid bitcoin address');
    });

    test("bitcoin_address_valid_test", () {
      expect(
          TextFieldValidation.bitcoinAddress(
              "3QCBtBm8oWAr7chAAprraV7rzp84cadvhF"),
          null);
    });

    /// Testing Ethereum address validation using [TextFieldValidation.ethereumAddress].
    test("ethereum_address_empty_validation_test", () {
      expect(TextFieldValidation.ethereumAddress(""),
          'Ethereum address is required');
    });

    test("ethereum_address_invalid_test", () {
      expect(TextFieldValidation.ethereumAddress("invalidEthereumAddress"),
          'Please enter a valid ethereum address');
    });

    test("ethereum_address_valid_test", () {
      expect(
          TextFieldValidation.ethereumAddress(
              "0xd87A780947eE3c0659877dBA7B01B10295D726a4"),
          null);
    });
    ...
    test("ripple_address_empty_validation_test", () {
      expect(
          TextFieldValidation.rippleAddress(""), 'Ripple address is required');
    });

    test("ripple_address_invalid_test", () {
      expect(TextFieldValidation.rippleAddress("invalidRippleAddress"),
    ...
    test("litecoin_address_empty_validation_test", () {
      expect(TextFieldValidation.litecoinAddress(""),
          'Litecoin address is required');
    });

    test("litecoin_address_invalid_test", () {
      expect(TextFieldValidation.litecoinAddress("invalidLitecoinAddress"),
    ...
    test("algorand_address_empty_validation_test", () {
      expect(TextFieldValidation.algorandAddress(""),
          'Algorand address is required');
    });

    test("algorand_address_invalid_test", () {
      expect(TextFieldValidation.algorandAddress("invalidAlgorandAddress"),
    ...
    test("tether_address_empty_validation_test", () {
      expect(
          TextFieldValidation.tetherAddress(""), 'Tether address is required');
    });

    test("tether_address_invalid_test", () {
      expect(TextFieldValidation.tetherAddress("invalidTetherAddress"),
          'Please enter a valid tether address');
    });

    /// Testing Monero address validation using [TextFieldValidation.moneroAddress].
    test("monero_address_empty_validation_test", () {
      expect(
          TextFieldValidation.moneroAddress(""), 'Monero address is required');
    });

    test("monero_address_invalid_test", () {
      expect(TextFieldValidation.moneroAddress("invalidMoneroAddress"),
